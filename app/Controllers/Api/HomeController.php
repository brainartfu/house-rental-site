...
class HomeController extends APIController
{
	...
	public function getFilters(Request $request){
        ...
        $filter_type = [
			...
			'home-typesec' => __('Home Type additional'),
	...
	public function search(Request $request){
		...
		if($posts['total'] > 0){
			...
			$temp->amenities = $amenities;
				...
				$home_typesec = $v->home_typesec;
				if(!empty($home_typesec)){
					$term_temp = get_term_by('id', $home_typesec);
					if($term_temp) {
						$temp->home_typesec = [
							'id' => $home_type,
							'link' => get_term_link($term_temp->term_name),
	...
	$home_typesec = $data->home_typesec;
            if(!empty($home_typesec)){
                $term = get_term_by('id', $home_typesec);
                if($term){
                    $data->home_typesec = [
                        'id' => $term->term_id,
                        'link' => get_term_link($term->term_name),
