...
class CarController extends Controller
{
    ...
    public function _sendEnquiryForm(Request $request)
    {
        ...
        if(in_array('admin', $enquiry_options)){
            ...
            send_mail(esc_html($email_from), esc_html($email_name), $admin->email, sprintf(__('[%s] Have a booking request from [Car ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($message));
        }
        if(in_array('partner', $enquiry_options)){
            $partner = get_user_by_id($post->author);
            send_mail(esc_html($email_from), esc_html($email_name), $partner->email, sprintf(__('[%s] Have a booking request from [Car ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($message));
        }
        if(in_array('customer', $enquiry_options)){
            send_mail(esc_html($email_from), esc_html($email_name), $email, sprintf(__('[%s] You have created a booking request for [Car ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($message));
        }
