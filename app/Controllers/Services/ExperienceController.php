...
class ExperienceController extends Controller
{
    ...
    public function _sendEnquiryForm(Request $request)
    {
        ...
        $content = sprintf(__('Name: %s'), $name);
        $content .= '<br />' . sprintf(__('Email: %s'), $email);
        $content .= '<br />' . sprintf(__('Message: %s'), $message);
        if(in_array('admin', $enquiry_options)){
            $admin = get_admin_user();
            send_mail(esc_html($email_from), esc_html($email_name), $admin->email, sprintf(__('[%s] Have a booking request from [Experience ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($content), $email);
        }
        if(in_array('partner', $enquiry_options)){
            $partner = get_user_by_id($post->author);
            send_mail(esc_html($email_from), esc_html($email_name), $partner->email, sprintf(__('[%s] Have a booking request from [Experience ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($content), $email);
        }
        if(in_array('customer', $enquiry_options)){
            send_mail(esc_html($email_from), esc_html($email_name), $email, sprintf(__('[%s] You have created a booking request for [Experience ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($content));
        }
