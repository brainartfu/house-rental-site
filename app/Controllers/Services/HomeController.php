    ...
    public function _getSearchResult()
    {
        ...
        if ($data['total'] > 0) {
            foreach ($data['results'] as $k => $item) {
				...
				$type_2 = get_term_by('id', $item->home_typesec);
                $type_name_2 = $type_2 ? get_translate($type_2->term_title) : '';
                
                $price_total = view('frontend.home.loop.price', [
                    'check_in' => request()->get('checkIn'),
                    'check_out' => request()->get('checkOut'),
                    'item' => $item
                ])->render();
                ...
                $locations[] = [
                    ...
                    'thumbnail' => get_attachment_url($item->thumbnail_id, [200, 200]),
                    'people' => $item->number_of_guest,
