...
{
    $html = '';
	 if (get_translate($item->location_postcode)) {
              $html .= get_translate($item->location_postcode) . ', ';
    }
    if (get_translate($item->location_city)) {
        $html .= get_translate($item->location_city) . ', ';
    }
    if (get_translate($item->location_state)) {
        $html .= get_translate($item->location_state);
    }
...
function get_shorted_address($item)
{
    $html = '';
...
function crop_image($path, $size = [200, 200])
{
    switch ($size) {
        case 'full':
            $size = [1200, 900];
