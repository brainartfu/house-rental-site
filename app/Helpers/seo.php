...
function seo_output()
{
    ...
    if (!isset($seo_item) || is_null($seo_item)) {
        return;
    }
...
function seo_encode($seo_text, $language_code, $post)
{
    if(empty($seo_text)){
        return '';
    }
    $encode = [
        '{{id}}' => $post->post_id ?? '',
        '{{title}}' => !empty($post->post_title) ? get_translate($post->post_title, $language_code) : '',
        '{{description}}' => !empty($post->post_description) ? get_translate($post->post_description, $language_code) : '',
        '{{separator}}' => '-',
