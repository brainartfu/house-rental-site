...
class Home extends Model
{
    ...
    public function countHomeInHomeType($home_type_id)
    {
	  ...
	  public function countHomeInHomeType2($home_type_id)
    {
        $sql = DB::table($this->getTable())->selectRaw("COUNT(*)");
        $sql->where('home_typesec', $home_type_id);
        $sql->whereRaw("status = 'publish'");
        $results = $sql->count();
        return $results;
    }
    ...
    public function getSearchResult($data)
    {
        ...
        if (!empty($data['home-type'])) {
		 ...
		 if (!empty($data['home-typesec'])) {
            $data['home-typesec'] = esc_sql($data['home-typesec']);
            $sql->whereRaw("home.home_typesec IN ({$data['home-typesec']})");
        }
