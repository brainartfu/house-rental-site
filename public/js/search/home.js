                        ...
                        if (res.status) {
                            ...
                            setTimeout(function () {
                                            ...
                                            if (res.total > 0) {
                                                ...
                                                locations.forEach(function (item) {
                                                    ...
                                                    if ($('.hh-map-price[data-id="' + item.post_id + '"]', base.search_container).length === 0) {
                                                        ...
                                                        let string = item.total_price;
                                                        let price = string.match("<span class=\"price-inner\">(.*) <span")[1];
                                                        let day = string.match("&nbsp;(.*) </span")[1];
                                                        let span_1 = document.createElement('span');
                                                        let span_2 = document.createElement('span');
														let span_3 = document.createElement('span');
                                                        span_1.className = 'price-content';
                                                        span_2.className = 'day-content';
														span_3.className = 'before-content';
                                                        el.className = 'marker';
                                                        span_1.innerHTML = price;
                                                       // span_2.innerHTML = day;
														span_3.innerHTML = '<i class="fa fa-map-pin fa-3x" aria-hidden="true"></i>';
                                                        el.append(span_3);
														el.append(span_1);
                                                        //el.append(span_2);
														let marker = new mapboxgl.Marker(el).setLngLat([item.lng, item.lat])
                                                                    .setPopup(
                                                                        new mapboxgl.Popup({ offset: 25 }) // add popups
                                                                        .setHTML('<div class="hh-map-popup">' +
                                                                '<div class="thumb"><a href="' + item.url + '"><img class="image-size" src="' + item.thumbnail + '" alt="' + item.title + '"/><div class="hh-map-price" data-id="' + 
                                                                item.post_id + '">'+ item.total_price +'</div></a></div>' + '<div class="hh-map-home-type" style="font-size:8px; padding: 10px 10px 0 10px;text-transform: uppercase;" data-id="' + item.post_id + '"><span class="codpopup">Codice: <b>' + item.post_id + '</b></span></div><!--<span style="font-size:12px">' + item.home_type + '</span></div>-->' +
                                                                '<div class="content"><a href="' + item.url + '">' + item.title + 
                                                                '</a><div style="border-top: 1px solid #dfdfdf; margin-top: 5px; margin-bottom:10px; width: 100%;"></div><div style="display: flex; align-items: center; justify-content: space-between;"><div><i class="fa fa-users mapicons"></i>' + item.people + 
                                                                '</div><div><i class="fa fa-bed mapicons"></i>' + item.bedrooms + 
                                                                '</div><div><i class="fa fa-bath mapicons"></i>' + item.bathrooms + '<div></div></div></div>')
                                                                        )
                                                            .addTo(base.mapObject);
                                                        base.allPopups.push(marker);
                                ...
                                $('.hh-map-price').click(function () {
                                    ...
                                    tt.parent().find('.hh-map-popup').css({'display': 'block'});
                                });
                                ...
                                $('.hh-search-results-map').on('click', function (ee) {
                                    ...
                                    if (!$(target).is('.hh-map-price')) {
                                        $('.hh-map-price').closest('.mapboxgl-popup').removeClass('max-zindex');
                                        $('.hh-map-price').parent().find('.hh-map-popup').show();
                                    }
                                });
