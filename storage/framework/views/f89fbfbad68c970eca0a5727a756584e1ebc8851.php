...
<html>
    ...
    <body>
        ...
        <script type="text/javascript" src="<?php echo e(asset('installer/js/jquery.js')); ?>"></script>
                ...
                $(document).ready(function(){
                   if($('#awe_import_demo').length){
                       $('#awe_import_demo').click(function(e){
                           e.preventDefault();
                           var t = $(this);
                           var cof = confirm('Are you sure want to install demo data?');
                           if(cof){
                               $('#awe-import-label').fadeIn();
                               $('#awe_import_demo').css({'opacity': '0.3'});
                               importDemo(1);
                               function importDemo(step = 1){
                                  $.post(t.data('action'), {'step' : step, '_token' : window.Laravel.csrfToken}, function (respon) {
                                       if(respon.next_step === 'final'){
                                           $('#awe_import_demo').css({'opacity': '0', 'visibility': 'hidden'});
                                           $('#awe-import-label .title').find('.loader').remove();
                                           $('#awe-import-label').append('<span class="item done">Completed!</span>');
                                       }else if(respon.next_step === 'imported') {
                                           $('#awe_import_demo').css({'opacity': '0', 'visibility': 'hidden'});
                                           $('#awe-import-label .title').find('.loader').remove();
                                           $('#awe-import-label').find('.item.done').remove();
                                           $('#awe-import-label').append('<span class="item done">'+ respon.label +'</span>');
                                       }else{
...
<?php /**PATH C:\xampp7\htdocs\resources\views/vendor/installer/layouts/master.blade.php ENDPATH**/ ?>